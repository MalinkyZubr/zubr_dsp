...
impl<const BUFFER_SIZE: usize> PipelineNodeOp<BufferArray<Complex<f32>, BUFFER_SIZE>, BufferArray<Complex<f32>, BUFFER_SIZE>, 1> for AMModulator<BUFFER_SIZE> {
    fn run_cpu(&mut self, input: &mut [BufferArray<Complex<f32>, BUFFER_SIZE>; 1], output: &mut BufferArray<Complex<f32>, BUFFER_SIZE>) -> Result<(), ()> {
        ...
        for output_sample in output.read_mut().iter_mut() {
            *output_sample = self.carrier_amplitude * (Complex::new(1.0, 0.0) + self.modulation_index * *output_sample);
        }
