...
impl NodeMasterReadData {
    ...
    pub fn update_analytics(&self, execution_time_ns: u64) {
        ...
        self.last_execution_instant
            .store(time::SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs(), std::sync::atomic::Ordering::Relaxed);
    }
    
    pub fn duration_since_execution_secs(&self) -> u64 {
        let current_time = time::SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs();
        current_time - self.last_execution_instant.load(std::sync::atomic::Ordering::Acquire)
    }
}
...
impl PipelineGraph {
    pub fn new(build_vector: Rc<RefCell<PipelineBuildVector>>) -> Self {
        ...
        for (id, name, node) in nodes {
            info!("\t{}: {} INPUTS: {}, OUTPUTS: {}", id, name, node.get_num_inputs(), node.get_num_outputs() );
...
mod tests {
    use super::*;
    use crate::engine::construction_layer::node_builder::PipelineParameters;
    ...
    impl CollectibleNode for MockNode {
        ...
        fn get_run_model(
            &self,
        ) -> crate::engine::structural::generic_pipeline_node::RunModel {
