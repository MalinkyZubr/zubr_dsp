...
use crate::engine::construction_layer::node_builder::{BuildingNode, PipelineBuildVector};
use crate::engine::structural::generic_node_operation::{PipelineSink, PipelineSource, PipelineNodeOp};
...
impl<I: Sharable, O: Sharable, const NI: usize, const NO: usize> NodeBuilder<I, O, NI, NO> {
    ...
    pub(in crate::engine::construction_layer) fn attach_to_recipe_input(
        &mut self,
        dest_id: usize,
    ) -> WrappedReceiver<O> {
    ...
    pub(in crate::engine::construction_layer) fn attach_to_recipe_output(
        &mut self,
    ...
    pub fn attach_standard<F: Sharable, const NIN: usize, const NON: usize>(
        ...
        let buff_size = self.build_vector.borrow().get_parameters().buff_size;
    ...
    pub fn add_pipeline_sink(
        ...
        let mut new_node: BuildingNode<O, (), 1, 0> =
            BuildingNode::new(name, new_id, buff_size);
        let (sender, receiver) = channel_wrapped::<O>(
            self.build_vector.borrow_mut().get_parameters().buff_size,
            self.node_predecessor.get_id(),
            new_node.get_id(),
    ...
    pub fn create_standalone_sink<T: Sharable>(
        ...
        let buff_size = build_vector.borrow().get_parameters().buff_size;
        let mut new_node = BuildingNode::new(name, new_id, buff_size);
        new_node.attach_step(Box::new(step));
        ...
        I: Unit,
    {
        ...
        let new_id = build_vector.borrow_mut().get_new_id();
        let buff_size = build_vector.borrow().get_parameters().buff_size;
        let mut start_node = BuildingNode::new(name, new_id, buff_size);
