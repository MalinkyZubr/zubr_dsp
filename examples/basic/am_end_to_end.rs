...
pub fn am_end_to_end_test() -> Result<(), String> {
    ...
    let mut step2: UnfinishedNodeBuilder<_, _, 1, 1> = step1.attach_standard("AmModulator".to_string(), AMModulator::<2048>::new(
    ...
    let mut step3: UnfinishedNodeBuilder<_, _, 1, 1> = step2.attach_standard("Chunker".to_string(), breaker);
    let mut step4 = step3.attach_series_deconstructor::<1>("deconstructor".to_string());
    let mut converter: UnfinishedNodeBuilder<_, _, 1, 1> = step4.attach_standard("converter".to_string(), RealToComplex::new());
    let mut step5: UnfinishedNodeBuilder<_, _, 1, 1> = converter.attach_standard("fft".to_string(), FFT::new());
    let mut step6: UnfinishedNodeBuilder<_, _, 1, 1> = step5.attach_standard("analytic filter".to_string(), FIRFilter::<_, 512>::new(tf_analytic::<f32, 128>()));
    let mut step7: UnfinishedNodeBuilder<_, _, 1, 1> = step6.attach_standard("ifft".to_string(), IFFT::new());
    let mut step8 = step7.attach_series_reconstructor::<1, 8>("reconstructor".to_string());
    let mut dechunker: UnfinishedNodeBuilder<_, _, 1, 1>= step8.attach_standard("dechunker".to_string(), combiner);
    let mut step9: UnfinishedNodeBuilder<_, _, 1, 1> = dechunker.attach_standard("Magnitude".to_string(), ComplexMagnitude::new());
    let mut downsampler: UnfinishedNodeBuilder<_, _, 1, 1> = step9.attach_standard("downsampler".to_string(), SimpleDownsampler::<2048, 36>::new());
    let mut step10: UnfinishedNodeBuilder<_, _, 1, 1> = downsampler.attach_standard("demodulator".to_string(), AMDemodulator::new(10.0, 0.5))
